...
func Connect(config RenExConfig) (Conn, error) {
	...
	if config.RenExBrokerVerifierAddress == "" {
		...
		case NetworkTestnet:
			...
			config.RenExSettlementAddress = "0xB560B9f0B9D7B5c3BAE2f6b5b380a5521f206b6e"
		case NetworkNightly:
...
func ConnectWs(config RenExConfig) (Conn, error) {
	switch config.Network {
	case NetworkMainnet:
		config.URI = "wss://mainnet.infura.io/ws"
	case "ropsten":
		config.URI = "wss://ropsten.infura.io/ws"
	case NetworkTestnet:
		config.URI = "wss://kovan.infura.io/ws"
	case "rinkeby":
		config.URI = "wss://rinkeby.infura.io/ws"
	default:
